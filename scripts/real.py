...
for patient_name in names:
  ...
  for i in range(1,9):
    atlas = "atlas-" + str(i)
    reg_path  = code_dir + '/results/reg-' + pat + '-nav/' + atlas + '/'
############### === define parameters
    p['output_dir'] 		    = os.path.join(code_dir, 'results/inv-' + pat + '/' + atlas + '-nav-ratio-0.5/');  	# results path
...
#p['pvec_path']          = code_dir + '/results/t16-case6/p-rec-forward.txt'                        # path to initial guess p vector (if none, use zero)
    ...
    p['k_gm_wm']            = 0.5                  # kappa ratio gm/wm (if zero, kappa=0 in gm)
