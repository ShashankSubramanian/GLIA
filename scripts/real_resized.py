...
for patient_name in names:
  ...
  for atlas in os.listdir(reg):
    if atlas[0] == "5": ### adni atlases
      reg_path                = reg + atlas + '/'
############### === define parameters
      p['n']                  = sz_int                        # grid resolution in each dimension
      p['output_dir'] 		    = os.path.join(code_dir, 'results/' + pat + '/tu/' + atlas + '-sz160/');  	# results path
      p['d1_path'] 			      = real_path + pat + '_tu_aff2jakob_' + sz + '.nc'
      p['d0_path']            = real_path + pat + '_c0Recon_aff2jakob_' + sz + '.nc'  # path to initial condition for tumor
#      p['d0_path']            = reg_path + '/c0Recon_transported.nc'                # path to initial condition for tumor

      p['atlas_labels']       = "[wm=6,gm=5,vt=7,csf=8,ed=2,nec=1,en=4]"# brats'[wm=6,gm=5,vt=7,csf=8,ed=2,nec=1,en=4]'
      p['a_seg_path']         = code_dir +  "/../data/adni-nc/" + sz + "/" + atlas + "_seg_aff2jakob_ants_" + sz + ".nc"
...
#      p['a_vt_path']          = reg_path + atlas + '_vt.nc' 
      p['mri_path'] 			    = code_dir + "/../data/adni-nc/" + sz + "/" + atlas + "_t1_aff2jakob_" + sz + ".nc"
      p['p_wm_path'] 			    = real_path + pat + '_wm_aff2jakob_' + sz + '.nc'           # patient brain data path
      p['p_gm_path'] 			    = real_path + pat + '_gm_aff2jakob_' + sz + '.nc'           # patient brain data path
      p['p_csf_path']         = real_path + pat +'_csf_aff2jakob_' + sz + '.nc'           # patient brain data path
      p['p_vt_path'] 			    = real_path + pat + '_vt_aff2jakob_' + sz + '.nc'           # patient brain data path
      p['obs_mask_path']  	  = real_path + pat +'_obs_aff2jakob_' + sz + '.nc'           # patient brain data path
