...
def combine_segmentations(healthy_seg, tumor_seg, out_seg, labels):
    try:
        print('loading base={}, overlay={} ... '.format(healthy_seg, tumor_seg), end='')
        healthy_seg_img = nibabel.load(healthy_seg)
        tumor_seg_img = nibabel.load(tumor_seg)
    except Exception as e:
        print('failed to load. Skipping.')
        return
    #healthy_seg_data = np.squeeze(healthy_seg_img.get_fdata())
    #tumor_seg_data = np.squeeze(tumor_seg_img.get_fdata())

    healthy_seg_data = healthy_seg_img.get_fdata()
    tumor_seg_data = tumor_seg_img.get_fdata()


    tu_en  = tumor_seg_data == labels['en'];
    tu_ed  = tumor_seg_data == labels['ed'];
    tu_nec = tumor_seg_data == labels['nec'];
    tu_wt  = np.logical_or.reduce((tu_en, tu_ed, tu_nec));
    tu_tc  = np.logical_or.reduce((tu_nec, tu_en));

    healthy_seg_data[tu_wt] = tumor_seg_data[tu_wt]
    #healthy_seg_data.astype(healthy_seg_img.get_data_dtype())
    healthy_seg_data.astype(int)

    fio.writeNII(healthy_seg_data, out_seg, ref_image=healthy_seg_img);
    print("saved new segmentation to {}".format(out_seg))
...
if __name__=='__main__':
    ...
    parser.add_argument ('-integrate_tumor_in_atlas',     action='store_true', help = 'integrates tumor labels into atlas image');
    parser.add_argument ('--patient_labels',      type = str,          help = 'patient labels');
    parser.add_argument ('--healthy_seg',         type=str, help = 'healthy segmentation for combining segmentations');
    parser.add_argument ('--tumor_seg',           type=str, help = 'tumor segmentation for combining segmentations');
    parser.add_argument ('--out_seg',             type=str, help = 'out  segmentation for combining segmentations');
    ...
    if args.integrate_tumor_in_atlas:
        print("integrating transported tumor segmentation into healthy atlas segmentation.")
        combine_segmentations(args.healthy_seg, args.tumor_seg, args.out_seg, patient_label_rev);
