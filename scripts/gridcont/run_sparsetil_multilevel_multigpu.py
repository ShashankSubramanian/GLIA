...
def sparsetil_gridcont_gpu(input, patient_data_paths, output_base_paths, job_path, job_idx = 0, use_gpu = True):
    ...
    for level, ii in zip(levels, range(len(levels))):
      for gpu_idx, (patient_data_path, output_base_path) in enumerate(zip(patient_data_paths, output_base_paths)):
...
#        filename = fname.split('/')[-1].split('.')[0]
        ...
        run_str = par.write_config(p, r, use_gpu, gpu_device_id = gpu_idx)
