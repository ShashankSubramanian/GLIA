...
def run_bratsreg(basedir, args, patient, atlas):
    ...
    #    cmd_preproc += ' -skip_patient'

    # symlinks to patient input files
    #FILES = ['patient_ed_wm.nc', 'patient_seg_tc.nc',  'patient_seg_csf.nii.gz', 'patient_seg_tc.nii.gz', 'patient_tc.nc',  'patient_ed_wm.nii.gz', 'patient_seg_wm_wt.nc', 'patient_seg_ed.nc', 'patient_seg_wm.nc',
    #'patient_tc.nii.gz', 'patient_csf.nc', 'patient_gm.nc', 'patient_seg_ed.nii.gz', 'patient_seg_wm.nii.gz', 'patient_wm.nc',  'patient_csf.nii.gz', 'patient_gm.nii.gz', 'patient_seg_gm.nii.gz', 'patient_seg_wm_wt.nii.gz','patient_wm.nii.gz', 'patient_ed.nc', 'patient_mask.nc','patient_seg_csf.nc', 'patient_seg.nc', 'patient_seg_wt.nn', 'patient_ed.nii.gz', 'patient_mask.nii.gz', 'patient_seg_gm.nc', 'patient_seg.nii.gz', 'patient_seg_wt.nii.gz']
    #cmd_symlink = ""
    #if args.skip_patient:
    #    cmd_symlink  =  "PWDO=${PWD} \n"
    #    cmd_symlink +=  "cd " + str(input_folder) + "\n"
    #    for file in FILES:
    #        if "patient" in file:
    #            cmd_symlink +=  "ln -sf " + os.path.join('../../'+str(patient)+'__jacob/input',file) + "  " + file +  " \n";
    #    cmd_symlink  +=  "cd ${PWDO} \n"
    #
...
if __name__=='__main__':
    ...
    parser.add_argument ('-skip_patient',   action = 'store_true', help = 'dont preprocess patient')
    parser.add_argument ('-patient',        type = str, help = 'patient name')
    parser.add_argument ('-atlas',          type = str, help = 'patient name')
    args = parser.parse_args();


    print('running for multiple patients')
    patient_list = os.listdir(args.patient_image_path)
    # atlas_list = os.listdir(args.atlas_image_path)
    #atlas_list = ['jacob', '0034Y02', '0056Y02', '0101Y02', '0115Y02', '0177Y02', '0222Y02', '0241Y02', '0258Y02', '0396Y02']
    ...
    for patient in patient_list:
        #process_patient = True
        for atlas in atlas_list:
            print('processing patient/atlas pair [{} / {}]'.format(patient, atlas))
            #args.patient_image_path = os.path.join(os.path.join(os.path.join(os.path.join(base_patient_image_path, patient), "data"), "affreg2-"+str(atlas)), patient + '_seg_dl_tu_256x256x256_aff2'+str(atlas)+'.nii.gz');
            args.patient_image_path = os.path.join(os.path.join(os.path.join(base_patient_image_path, patient), "affreg2-"+str(atlas)), patient + '_seg_dl_tu_256x256x256_aff2'+str(atlas)+'.nii.gz');
            if atlas == 'jacob':
                #args.atlas_image_path = os.path.join(os.path.join(base_atlas_image_path, atlas), 'jakob_stripped_with_cere_lps_256x256x256.nii.gz');
                args.atlas_image_path = os.path.join(os.path.join(base_atlas_image_path, atlas), 'jakob_segmented_with_cere_lps_256x256x256.nii.gz');
                args.atlas_t1_image   = os.path.join(os.path.join(base_atlas_image_path, atlas), 'jakob_stripped_with_cere_lps_256x256x256.nii.gz');
            else:
                args.atlas_image_path = os.path.join(os.path.join(base_atlas_image_path, atlas), atlas + '_segmented.nii.gz');
                args.atlas_t1_image   = os.path.join(os.path.join(base_atlas_image_path, atlas), atlas + '_cbq_n3.nii.gz');
            args.results_directory = os.path.join(os.path.join(base_results_dir, patient), "{}_regto_{}".format(patient,atlas));

            #args.skip_patient = False if process_patient else True;
            run_bratsreg(basedir, args, patient, atlas);
            #process_patient = False;
        # f.close()

    # else if patient is not None and atlas is not None:
        # run_bratsreg(basedir, args, patient, atlas);
