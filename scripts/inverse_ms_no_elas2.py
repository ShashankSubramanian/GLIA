...
bias_list = [1e-4, 2.5e-4, 5e-4, 7.5e-4, 1e-3] 
#bias_list = [5e-4] 
...
for i in range(1,3):
  ...
  for (j, bias) in enumerate(bias_list):
    ...
    res_dir = os.path.join(scratch, 'results/syn_results/true_p_true_m/ms_inv_160/'+syn+'/bias%d-2'%j)
    ...
    params_in = {}
    params_in['k'] = (0.1, 0.0001, 0.3)
    params_in['rho'] = (10.0, 1.0, 30.0)
    ...
    params_in['ox_hypoxia'] = (0.5, 0.1, 0.9)
    params_in['death_rate'] = (3.0, 0.1, 14.0)
    params_in['alpha_0'] = (1.0, 0.01, 3.0)
    params_in['ox_consumption'] = (5.0, 0.1, 30.0)
    params_in['ox_source'] = (3.0, 0.1, 30.0)
    params_in['beta_0'] = (1.0, 0.01, 3.0)
    ...
    with open(job_name, 'w') as f:
      ...
      cmd = "python -u "+os.path.join(code_dir, 'scripts', 'multispecies', 'run_inverse_ms_no_elas_16_obs_bias5.py')+" -p "+os.path.join(pat_dir)+" -r "+os.path.join(res_dir)+" -lb "+str_lbs+" -ub "+str_ubs+ " -i "+str_inits+" -inv "+str_inv_params+" -total "+str_params+" -sigma "+str(sigma)+" -vel "+v_dir+" -n "+str(resolution)+" -b "+str(bias)+" \n"
