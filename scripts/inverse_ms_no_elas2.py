...
for i in range(1,9):
  ...
  v_dir = os.path.join(scratch, 'results/syndata/'+syn+'/C1_me_fwd_vel/')
  ...
  if os.path.exists(recon_file):
    with open(recon_file, 'r') as f:
      lines = f.readlines()
      l = lines[1].split(" ")
      params_in['k'] = (float(l[0]), 0.001, 0.8)
      ...
      params_in['ox_source'] = (float(l[6]), 0.1, 20.0)
  ...
  params_in['invasive_thres'] = (-2, -5.0, -1.0)
  print(params_in)
  ...
  with open(job_name, 'w') as f:
    ...
    #cmd = "python -u "+os.path.join(code_dir, 'scripts', 'multispecies', 'run_inverse_ms_no_elas_16_obs_bias7.py')+" -p "+os.path.join(pat_dir)+" -r "+os.path.join(res_dir)+" -lb "+str_lbs+" -ub "+str_ubs+ " -i "+str_inits+" -inv "+str_inv_params+" -total "+str_params+" -sigma "+str(sigma)+" -vel "+v_dir+" -n "+str(resolution)+" -b "+str(bias)+" \n"
