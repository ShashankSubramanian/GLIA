...
def createCmdLineReg(param):
        ...
        if 'num_images' in param:
                cmd += ' -mtc ' + str(param['num_images']) + " $DATA_DIR/patient_vt.nii.gz $DATA_DIR/patient_gm.nii.gz $DATA_DIR/patient_ed_wm_csf.nii.gz" 
                cmd += ' -mrc ' + str(param['num_images']) + " $DATA_DIR/atlas_vt.nii.gz $DATA_DIR/atlas_gm.nii.gz $DATA_DIR/atlas_wm_csf.nii.gz" 
...
def createCmdLineTransport(param, input_filename, output_filename, labels=None):
        '''
...
##############################################################################
def createJobsubFile(cmd,param):
    ...
    bash_file.write("#SBATCH -n " + str(param['mpi_pernode']) + "\n");
    if param['compute_sys'] == 'lonestar':
    ...
    bash_file.write("#SBATCH --mail-type=all\n\n\n");            
