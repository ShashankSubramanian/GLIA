...
def run_multispecies_inversion(pat_dir, res_dir, init_vec, lb_vec, ub_vec, inv_params, list_vars, is_syn, sigma, v_dir, n, bias):
  ...
  fun = lambda x : create_cma_output(x, pat_dir, res_dir, cma_lb, cma_ub, init_vec, inv_params, list_vars, v_dir, n, bias)
  ...
  with open(recon_file, 'w') as f:
    ...
    for i in inv_params:
      var += i+' '
    var += '\n'
    for (i, comp) in enumerate(out_inv):
      if inv_params[i] == 'invasive_thres':
        var += str(10**comp)+' '
      else:
        var += str(comp)+' '
    f.write(var)
...
def create_cma_output(solutions, pat_dir, res_dir, lb_vec, ub_vec, init_vec, inv_params, list_vars, v_dir, n, bias):
  ...
  for i in range(num_batch):
    ...
    #os.wait()
    px_en = np.linalg.norm((0.5*dat_en_true).flatten())**2
    ...
    for j in range(num_devices):
      ...
      diff_en = w_en * np.linalg.norm((0.5*dat_en_true - dat_en_rec).flatten())**2
...
if __name__ == "__main__":
  ...
  r_args.add_argument('-b', '--bias', type=float, help = 'bias term', required = True)
  ...
  bias = args.bias
  print(sigma) 
  run_multispecies_inversion(pat_dir, res_dir, init_vec, lb_vec, ub_vec, inv_params, list_vars, True, sigma, v_dir, n, bias) 
