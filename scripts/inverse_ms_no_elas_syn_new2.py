...
for i in range(1,9):
  ...
  pat_fwd_dir = os.path.join(scratch, 'results/syn_results/me_inv_160/'+syn+'/fwd_me/') 
  res_dir = os.path.join(scratch, 'results/syn_results/ms_inv_160/'+syn+'_3/')
  ...
  with open(job_name, 'w') as f:
    ...
    cmd = "python -u "+os.path.join(code_dir, 'scripts', 'multispecies', 'run_inverse_ms_no_elas_16_obs_nocoeff2.py')+" -p "+os.path.join(pat_dir)+" -r "+os.path.join(res_dir)+" -lb "+str_lbs+" -ub "+str_ubs+ " -i "+str_inits+" -inv "+str_inv_params+" -total "+str_params+" -sigma "+str(sigma)+" -vel "+v_dir+" -n "+str(resolution)+" \n"
