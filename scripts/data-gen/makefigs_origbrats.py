...
if __name__=='__main__':
    ...
    parser.add_argument ('-pred_dir',                                        type = str, help = 'path to predictions')
    ...
    for index, row in test_df.iterrows():
        ...
        pred_p     = os.path.join(args.pred_dir, os.path.basename(abnormal_p).split(".nii.gz")[0]+"_seg.nii.gz")
        orig_p     = os.path.join(os.path.dirname(abnormal_p), str(patient) + "_seg_dl_tu__256x256x124.nii.gz")
        out_p      = os.path.join(args.results_directory, patient);
        ...
        for img, capt, fname in zip([abnormal, pred, orig], ['abnormal (syn)', 'normal (pred)', 'abnormal (brats)'],
            ['abnormal_syn_'+cm_where+'_ax-slice-'+str(z), 'normal_pred_'+cm_where+'_ax-slice-'+str(z), 'abnormal_brats_'+cm_where+'_ax-slice-'+str(z)]):
        ...
        for img, capt, fname in zip([abnormal, pred, orig], ['abnormal (syn)', 'normal (pred)',  'abnormal (brats)'],
            ['abnormal_syn_'+cm_where+'_sag-slice-'+str(x), 'normal_pred_'+cm_where+'_sag-slice-'+str(x), 'abnormal_brats_'+cm_where+'_sag-slice-'+str(x)]):
            aax = fig.add_subplot(1, 1, 1);
            aax.tick_params(axis='both', which='both', bottom=False, top=False, left=False, labelleft=False, labelbottom=False);
        ...
        for img, capt, fname in zip([abnormal, pred, orig], ['abnormal (syn)', 'normal (pred)',  'abnormal (brats)'],
            ['abnormal_syn_'+cm_where+'_cor-slice-'+str(y), 'normal_pred_'+cm_where+'_cor-slice-'+str(y), 'abnormal_brats_'+cm_where+'_cor-slice-'+str(y)]):
            aax = fig.add_subplot(1, 1, 1);
            aax.tick_params(axis='both', which='both', bottom=False, top=False, left=False, labelleft=False, labelbottom=False);
