        ...
        for level in levels:
            ...
            if os.path.exists(os.path.join(level_path, logfile)):
                with open(os.path.join(level_path, logfile), 'r') as f:
                    ...
                    for line in lines:
                        f_empty = False;
                        if "rho/kappa inversion with scaled L2 solution guess" in line:
                        ...
                        if "reconstructed p norm:" in line:
                            pnorm = float(line.split("reconstructed p norm: ")[-1].split()[0])
                        ...
                        if "rel. l2-error at observation points:" in line:
                            p_dict['l2Oc1'] = float(line.split(": ")[-1].split()[0])
                        if "l2-error in reconstruction:" in line:
                            p_dict['l2c1'] = float(line.split(":")[-1].split()[0])
