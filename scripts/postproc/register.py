...
def create_sbatch_header(results_path, idx, compute_sys='longhorn'):
    bash_filename = results_path + "/registration-job-" + str(idx) + ".sh"
    ...
    bash_file.write("#SBATCH -J registration\n");
    bash_file.write("#SBATCH -o " + results_path + "/reg_log_" + str(idx) + ".txt\n")
...
### write out the netcdf files also
    altas_mat_img = 0 * altas_seg
    altas_mat_img[altas_seg == 7] = 1
    createNetCDFFile(results_path + "/" + atlas_name + "_vt.nc", 256 * np.ones(3), np.transpose(altas_mat_img))
    altas_mat_img = 0 * altas_seg
    altas_mat_img[altas_seg == 8] = 1
    createNetCDFFile(results_path + "/" + atlas_name + "_csf.nc", 256 * np.ones(3), np.transpose(altas_mat_img))
    altas_mat_img = 0 * altas_seg
    altas_mat_img[altas_seg == 5] = 1
    createNetCDFFile(results_path + "/" + atlas_name + "_gm.nc", 256 * np.ones(3), np.transpose(altas_mat_img))
    altas_mat_img = 0 * altas_seg
    altas_mat_img[altas_seg == 6] = 1
    createNetCDFFile(results_path + "/" + atlas_name + "_wm.nc", 256 * np.ones(3), np.transpose(altas_mat_img))
    

def register(claire_bin_path, results_path, atlas_name, pat_path, patient_name, bash_filename, idx, multigpu = False):
    ...
    cmd += "ibrun " + claire_bin_path + "/claire -mrc 3 " + results_path + "/" + atlas_name + "_vt.nii.gz " + results_path + "/" + atlas_name \
                + "_gm.nii.gz " + results_path + "/" + atlas_name + "_wm_csf.nii.gz "\
                + "-mtc 3 " + pat_path + "/" + patient_name + "_vt.nii.gz " + pat_path + "/" + patient_name + "_gm.nii.gz " + pat_path + "/" + patient_name + "_wm_csf.nii.gz " \
                + "-mask " + pat_path + "/" + patient_name + "_mask.nii.gz " \
...
# transport
def transport(claire_bin_path, results_path, trans_image, name, bash_filename, idx, r2t=False, multigpu=False):
    ...
    if r2t == True:
      if name.find("labels") is not -1:
        cmd += "ibrun " + claire_bin_path + "/clairetools -v1 " + results_path + "/velocity-field-x1.nii.gz -v2 " + results_path + "/velocity-field-x2.nii.gz -v3 " + results_path + "/velocity-field-x3.nii.gz -r2t -tlabelmap -labels 0,1,2,4,5,6,7,8 -ifile "\
                + trans_image + " -xfile " + transport_image + " -iporder 1" + " > " + results_path + "/transport_log_" + name + ".txt &";
    else:
### enable -r2t if reference to transport is needed. keep disabled for now.
      if name.find("labels") is not -1:
        ...
        cmd += "ibrun " + claire_bin_path + "/clairetools -v1 " + results_path + "/velocity-field-x1.nii.gz -v2 " + results_path + "/velocity-field-x2.nii.gz -v3 " + results_path + "/velocity-field-x3.nii.gz -ifile "\
                + trans_image + " -xfile " + transport_image + " -deformimage -iporder 1" + " > " + results_path + "/transport_log_" + name + ".txt &";
