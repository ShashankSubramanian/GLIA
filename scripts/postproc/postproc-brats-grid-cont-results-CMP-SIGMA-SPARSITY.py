...
if __name__=='__main__':
    ...
    col_names = ["BraTS18ID", "level", "sigma", "s-lvl", "rho-inv", "k-inv", "l2Oc1", "l2c1(TC,s)", "l2c1(TC,#1,..,#n)", "l2c1", "sparsity", "np", "#comp", "xcm-dist", "exec-time", "age", "srvl[]", "dice_tc8", "#wt/#b", "#ed/#b", "#tc/#b", "#c0/#b", "I_EDc1", "I_TCc1", "I_B\WTc1", "Ic0/Ic1","comment"]
    ...
    BASE = args.dir
    for dir in DIRS:
        if not "concomp" in dir:
            continue;
        sigma          = str(float(dir.split("concomp-gauss-selection-sigma-")[-1].split("-")[0])) + 'h';
        sparsity_level = int(dir.split("sparsity-")[-1])
        args.dir = os.path.join(BASE, dir);
    ...
    df = df.sort_values(by=['BraTS18ID','level', 'sigma', 's-lvl']);
    # df = df.sort_values(by=['s-lvl','BraTS18ID','level', 'sigma']);
    df.to_csv('grid-cont-analysis_' + str(args.dir) + '.csv')
    ...
    if True:
        ...
        HTML += "\n\n"
    # write html file
    filename_html = 'brats-grid-cont-analysis(levels)_' + str(args.dir) + '.html'
    ...
    HTML += "\n\n"
    # write html file
    filename_html = 'brats-grid-cont-analysis(128)_' + str(args.dir) + '.html'
    html_file = open(filename_html, 'w')
    html_file.write(HTML)
    html_file.close()
    ...
    HTML += "\n\n"
    # write html file
    filename_html = 'brats-grid-cont-analysis(256)_' + str(args.dir) + '.html'
    html_file = open(filename_html, 'w')
    html_file.write(HTML)
    html_file.close()
    ...
    data256['srvl[]'].cat.categories = [-1, 3,2,1]   # alpha numeric sorting: [NA, long, mid, short] -> [-1, 2,1,0]
