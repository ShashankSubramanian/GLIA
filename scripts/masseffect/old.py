...
#names     = ["ABO", "AMH", "ALU"]
names     = ["ALU"]
scratch   = "/scratch1/05027/shas1693/"
##patient_name = "ABO"
for patient_name in names:
  ...
  at_list_f   = open(code_dir + 'results/' + pat + '/atlas_list', 'r')
  at_list     = at_list_f.readlines()
  at_list     = at_list[0:16]
  for atlas in at_list:
    atlas = atlas.strip("\n")
    if atlas[0] == "5": ### adni atlases
      reg_path  = reg + atlas + '/'
############### === define parameters
      p['output_dir'] 		    = os.path.join(code_dir, 'results/' + pat + '/tu/' + atlas + '-rholb/');  	# results path
...
#      p['a_vt_path']          = reg_path + atlas + '_vt.nc' 
      ...
      p['a_seg_path']         = scratch + "/adni-nc/" + atlas + "_seg_aff2jakob_ants.nc"
      p['mri_path'] 			    = scratch + "/adni-nc/" + atlas + "_t1_aff2jakob.nc"
...
############### === define run configuration
      ...
      r['compute_sys'] 		    = 'frontera'         			# TACC systems are: maverick2, frontera, stampede2, longhorn
