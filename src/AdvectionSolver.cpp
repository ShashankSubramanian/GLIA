...
TrapezoidalSolver::TrapezoidalSolver(std::shared_ptr<Parameters> params, std::shared_ptr<Tumor> tumor, std::shared_ptr<SpectralOperators> spec_ops) : AdvectionSolver(params, tumor, spec_ops) {
  PetscErrorCode ierr = 0;
...
ImplicitEulerSolver::ImplicitEulerSolver(std::shared_ptr<Parameters> params, std::shared_ptr<Tumor> tumor, std::shared_ptr<SpectralOperators> spec_ops) : AdvectionSolver(params, tumor, spec_ops) {
  PetscErrorCode ierr = 0;
  ierr = MatCreateShell(PETSC_COMM_WORLD, params->grid_->nl_, params->grid_->nl_, params->grid_->ng_, params->grid_->ng_, ctx_.get(), &A_);
...
PetscErrorCode advSolverKSPMonitor(KSP ksp, PetscInt its, PetscReal rnorm, void *ptr) {
  ...
  std::stringstream s;
  if (its == 0) {
    s << std::setw(3) << " KSP: computing solution of advection system (tol=" << std::scientific << std::setprecision(5) << reltol << ")";
...
SemiLagrangianSolver::SemiLagrangianSolver(std::shared_ptr<Parameters> params, std::shared_ptr<Tumor> tumor, std::shared_ptr<SpectralOperators> spec_ops) : AdvectionSolver(params, tumor, spec_ops) {
  PetscErrorCode ierr = 0;
...
// three versions of interpolation - multi-CPU, single-GPU, and multi-GPU (<none>, CUDA, MPICUDA)
  ...
  m_texture_ = gpuInitEmptyTexture(params->grid_->n_);
  cudaMalloc((void **)&temp_interpol1_, sizeof(float) * params->grid_->n_[0] * params->grid_->n_[1] * params->grid_->n_[2]);
  cudaMalloc((void **)&temp_interpol2_, sizeof(float) * params->grid_->n_[0] * params->grid_->n_[1] * params->grid_->n_[2]);

#ifndef SINGLE
  // more temp arrays to store a single precision copy of petsc vectors
  cudaMalloc((void **)&temp_1_, sizeof(float) * params->grid_->n_[0] * params->grid_->n_[1] * params->grid_->n_[2]);
  cudaMalloc((void **)&temp_2_, sizeof(float) * params->grid_->n_[0] * params->grid_->n_[1] * params->grid_->n_[2]);
  cudaMalloc((void **)&temp_3_, sizeof(float) * params->grid_->n_[0] * params->grid_->n_[1] * params->grid_->n_[2]);
#endif

  coords_ = std::make_shared<VecField>(params->grid_->nl_, params->grid_->ng_);
...
#else
  n_alloc_ = accfft_ghost_xyz_local_size_dft_r2c(params->grid_->plan_, n_ghost_, isize_g_, istart_g_);  // memory allocate
  ...
  interp_plan_scalar_->allocate(params->grid_->nl_, 1);
  interp_plan_vector_ = std::make_shared<InterpPlan>(n_alloc_);
  interp_plan_vector_->allocate(params->grid_->nl_, 3);
#else
  interp_plan_ = std::make_shared<InterpPlan>();
  interp_plan_->allocate(params->grid_->nl_, m_dofs_, 2);  // allocate memory for two sets of plans - scalars and vectors
#endif
#endif
  int factor = 3;
  ierr = VecCreate(PETSC_COMM_WORLD, &query_points_);
  ierr = VecSetSizes(query_points_, factor * params->grid_->nl_, factor * params->grid_->ng_);
...
// Interpolation
  ...
  accfft_get_ghost_xyz(params->grid_->plan_, n_ghost_, isize_g_, in_ptr, scalar_field_ghost_);  // populate scalar ghost field with input
  interp_plan_scalar_->interpolate(scalar_field_ghost_, 1, params->grid_->n_, params->grid_->isize_, params->grid_->istart_, params->grid_->nl_, n_ghost_, out_ptr, params->grid_->c_dims_, params->grid_->c_comm_, t.data());
  ...
  copyDoubleToFloatCuda(temp_1_, in_ptr, params->grid_->nl_);
  gpuInterp3D(temp_1_, &query_ptr[0], &query_ptr[params->grid_->nl_], &query_ptr[2 * params->grid_->nl_], temp_1_, temp_interpol1_, temp_interpol2_, params->grid_->n_, m_texture_, params->tu_->interpolation_order_,
              (float *)t.data());
  ...
  ierr = VecGetArray(output, &out_ptr); CHKERRQ(ierr);
  accfft_get_ghost_xyz(params->grid_->plan_, n_ghost_, isize_g_, in_ptr, scalar_field_ghost_);  // populate scalar ghost field with input
  interp_plan_->interpolate(scalar_field_ghost_, params->grid_->n_, params->grid_->isize_, params->grid_->istart_, params->grid_->nl_, n_ghost_, out_ptr, params->grid_->c_dims_, params->grid_->c_comm_, t.data(), 0);
  ierr = VecRestoreArray(input, &in_ptr); CHKERRQ(ierr);
  ierr = VecRestoreArray(output, &out_ptr); CHKERRQ(ierr);
#endif

  self_exec_time += MPI_Wtime();
  accumulateTimers(ctx_->params_->tu_->timers_, t, self_exec_time);
  e.addTimings(t);
  e.stop();
  PetscFunctionReturn(ierr);
}
...
PetscErrorCode SemiLagrangianSolver::interpolate(std::shared_ptr<VecField> output, std::shared_ptr<VecField> input) {
...
#ifdef SINGLE
  gpuInterpVec3D(ix_ptr, iy_ptr, iz_ptr, &query_ptr[0], &query_ptr[params->grid_->nl_], &query_ptr[2 * params->grid_->nl_], ox_ptr, oy_ptr, oz_ptr, temp_interpol1_, temp_interpol2_, params->grid_->n_,
                 m_texture_, params->tu_->interpolation_order_, (float *)t.data());
#else
  copyDoubleToFloatCuda(temp_1_, ix_ptr, params->grid_->nl_);
  copyDoubleToFloatCuda(temp_2_, iy_ptr, params->grid_->nl_);
  copyDoubleToFloatCuda(temp_3_, iz_ptr, params->grid_->nl_);
  gpuInterpVec3D(temp_1_, temp_2_, temp_3_, &query_ptr[0], &query_ptr[params->grid_->nl_], &query_ptr[2 * params->grid_->nl_], temp_1_, temp_2_, temp_3_, temp_interpol1_, temp_interpol2_, params->grid_->n_,
                 m_texture_, params->tu_->interpolation_order_, (float *)t.data());
  copyFloatToDoubleCuda(ox_ptr, temp_1_, params->grid_->nl_);
  ...
  for (int i = 0; i < 3; i++) {
    accfft_get_ghost_xyz(params->grid_->plan_, n_ghost_, isize_g_, &query_ptr[i * params->grid_->nl_], &vector_field_ghost_[i * nl_ghost]);  // populate vector ghost field with input
  }
// Interpolation
#ifdef MPICUDA
  interp_plan_vector_->interpolate(vector_field_ghost_, 3, params->grid_->n_, params->grid_->isize_, params->grid_->istart_, params->grid_->nl_, n_ghost_, query_ptr, params->grid_->c_dims_, params->grid_->c_comm_, t.data());
#else
  interp_plan_->interpolate(vector_field_ghost_, params->grid_->n_, params->grid_->isize_, params->grid_->istart_, params->grid_->nl_, n_ghost_, query_ptr, params->grid_->c_dims_, params->grid_->c_comm_, t.data(), 1);
#endif
...
// single-GPU
#if defined(CUDA) && !defined(MPICUDA)
  ierr = VecWAXPY(work_field_->x_, -dt / params->grid_->h_[0], velocity->x_, coords_->x_); CHKERRQ(ierr);
  ierr = VecWAXPY(work_field_->y_, -dt / params->grid_->h_[1], velocity->y_, coords_->y_); CHKERRQ(ierr);
  ierr = VecWAXPY(work_field_->z_, -dt / params->grid_->h_[2], velocity->z_, coords_->z_); CHKERRQ(ierr);
...
// only CPU (MPI)
  ...
  ierr = VecGetArray(query_points_, &query_ptr); CHKERRQ(ierr);
  int64_t ptr;
  for (int i1 = 0; i1 < params->grid_->isize_[0]; i1++) {
    for (int i2 = 0; i2 < params->grid_->isize_[1]; i2++) {
      for (int i3 = 0; i3 < params->grid_->isize_[2]; i3++) {
        x1 = params->grid_->h_[0] * static_cast<ScalarType>(i1 + params->grid_->istart_[0]);
        x2 = params->grid_->h_[1] * static_cast<ScalarType>(i2 + params->grid_->istart_[1]);
        x3 = params->grid_->h_[2] * static_cast<ScalarType>(i3 + params->grid_->istart_[2]);

        ptr = i1 * params->grid_->isize_[1] * params->grid_->isize_[2] + i2 * params->grid_->isize_[2] + i3;
...
#ifdef MPICUDA
  ierr = VecGetArray(query_points_, &query_ptr); CHKERRQ(ierr);
  interp_plan_scalar_->scatter(1, params->grid_->n_, params->grid_->isize_, params->grid_->istart_, params->grid_->nl_, n_ghost_, query_ptr, params->grid_->c_dims_, params->grid_->c_comm_, t.data());
  interp_plan_vector_->scatter(3, params->grid_->n_, params->grid_->isize_, params->grid_->istart_, params->grid_->nl_, n_ghost_, query_ptr, params->grid_->c_dims_, params->grid_->c_comm_, t.data());
  ierr = VecRestoreArray(query_points_, &query_ptr); CHKERRQ(ierr);
#elif !defined(CUDA)
  ierr = VecGetArray(query_points_, &query_ptr); CHKERRQ(ierr);
  interp_plan_->scatter(params->grid_->n_, params->grid_->isize_, params->grid_->istart_, params->grid_->nl_, n_ghost_, query_ptr, params->grid_->c_dims_, params->grid_->c_comm_, t.data());
  ierr = VecRestoreArray(query_points_, &query_ptr); CHKERRQ(ierr);
#endif
...
// single-GPU
#if defined(CUDA) && !defined(MPICUDA)
  ierr = VecAYPX(work_field_->x_, -0.5 * dt / params->grid_->h_[0], coords_->x_); CHKERRQ(ierr);
  ierr = VecAYPX(work_field_->y_, -0.5 * dt / params->grid_->h_[1], coords_->y_); CHKERRQ(ierr);
  ierr = VecAYPX(work_field_->z_, -0.5 * dt / params->grid_->h_[2], coords_->z_); CHKERRQ(ierr);
  ierr = VecAXPY(work_field_->x_, -0.5 * dt / params->grid_->h_[0], velocity->x_); CHKERRQ(ierr);
  ierr = VecAXPY(work_field_->y_, -0.5 * dt / params->grid_->h_[1], velocity->y_); CHKERRQ(ierr);
  ierr = VecAXPY(work_field_->z_, -0.5 * dt / params->grid_->h_[2], velocity->z_); CHKERRQ(ierr);
  ierr = work_field_->getIndividualComponents(query_points_); CHKERRQ(ierr);
// multi-GPU
#elif defined(MPICUDA)
  ierr = velocity->getComponentArrays(vx_ptr, vy_ptr, vz_ptr);
