...
TumorSolverInterface::TumorSolverInterface (
  ...
  , inv_solver_ ()
{
  if (params != nullptr and spec_ops != nullptr)
    initialize (params, spec_ops, phi, mat_prop);
}
...
PetscErrorCode TumorSolverInterface::setParams(std::shared_ptr<TumorParameters> tumor_params) {
  ...
  if(tumor_params != nullptr) {
    ...
    {ierr = tuMSGwarn("Error: (setParams) Tumor Parameter must not be NULL. Exiting."); CHKERRQ(ierr); PetscFunctionReturn(1); }
  }
  PetscFunctionReturn(ierr);
}
...
PetscErrorCode TumorSolverInterface::setParams (Vec p, std::shared_ptr<TumorParameters> tumor_params = {}) {
  ...
  if(model_changed) {
    ...
    if (np_changed) {ierr = derivative_operators_->reset(p, pde_operators_, params_, tumor_); CHKERRQ(ierr);}
    if (nt_changed) {ierr = pde_operators_->reset(params_, tumor_); CHKERRQ(ierr);}
  }
...
PetscErrorCode TumorSolverInterface::updateTumorCoefficients (Vec wm, Vec gm, Vec glm, Vec csf, Vec bg) {
  ...
  ierr = tumor_->phi_->setValues(tumor_->mat_prop_); CHKERRQ(ierr); // update the phi values, i.e., update the filter
  ierr = pde_operators_->diff_solver_->precFactor(); CHKERRQ(ierr); // need to update prefactors for diffusion KSP preconditioner, as k changed
