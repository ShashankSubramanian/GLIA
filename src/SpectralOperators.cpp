...
PetscErrorCode SpectralOperators::computeDivergence (Vec div, Vec dx, Vec dy, Vec dz, double *timers) {
    ...
    ScalarType *div_ptr, *dx_ptr, *dy_ptr, *dz_ptr;
        ...
        if (fft_mode_ == ACCFFT) {
            ...
            multiplyXWaveNumberCuda ((CudaComplexType*) x_hat_, (CudaComplexType*) x_hat_, osize_);
            // backwards transform
            cufft_status = cufftExecuteC2R (plan_c2r_, (CufftComplexType*) x_hat_, (CufftScalarType*) div_ptr_);
            cufftCheckError (cufft_status);
            cudaDeviceSynchronize ();
            ...
            multiplyYWaveNumberCuda ((CudaComplexType*) x_hat_, (CudaComplexType*) x_hat_, osize_);
            // backwards transform
            cufft_status = cufftExecuteC2R (plan_c2r_, (CufftComplexType*) x_hat_, (CufftScalarType*) d1_ptr_);
            cufftCheckError (cufft_status);
            cudaDeviceSynchronize ();
            ...
            multiplyZWaveNumberCuda ((CudaComplexType*) x_hat_, (CudaComplexType*) x_hat_, osize_);
            // backwards transform
            cufft_status = cufftExecuteC2R (plan_c2r_, (CufftComplexType*) x_hat_, (CufftScalarType*) d1_ptr);
            cufftCheckError (cufft_status);
            cudaDeviceSynchronize ();
...
int SpectralOperators::weierstrassSmoother (ScalarType * Wc, ScalarType *c, std::shared_ptr<NMisc> n_misc, ScalarType sigma) {
    ...
    ScalarType sum_f_local = 0., sum_f = 0;
        ...
        computeWeierstrassFilterCuda (f_, &sum_f_local, sigma, isize);
    #else
        ...
        for (int i = 0; i < isize[0]; i++)
            for (int j = 0; j < isize[1]; j++)
                for (int k = 0; k < isize[2]; k++) {
                    ...
                    f_[ptr] = std::exp((-X * X - Y * Y - Z * Z) / sigma / sigma / 2.0)
                            + std::exp((-Xp * Xp - Yp * Yp - Zp * Zp) / sigma / sigma / 2.0);

                    f_[ptr] += std::exp((-Xp * Xp - Y * Y - Z * Z) / sigma / sigma / 2.0)
                            + std::exp((-X * X - Yp * Yp - Z * Z) / sigma / sigma / 2.0);

                    f_[ptr] += std::exp((-X * X - Y * Y - Zp * Zp) / sigma / sigma / 2.0)
                            + std::exp((-Xp * Xp - Yp * Yp - Z * Z) / sigma / sigma / 2.0);

                    f_[ptr] += std::exp((-Xp * Xp - Y * Y - Zp * Zp) / sigma / sigma / 2.0)
                            + std::exp((-X * X - Yp * Yp - Zp * Zp) / sigma / sigma / 2.0);

                    if (f_[ptr] != f_[ptr])
                        f_[ptr] = 0.; // To avoid Nan
                    sum_f_local += f_[ptr];
                }
    #endif
    ...
    ScalarType normalize_factor = 1. / (sum_f * hx * hy * hz);
        ...
        // cublas for vec scale
        PetscCUBLASGetHandle (&handle);
        status = cublasScale (handle, isize[0] * isize[1] * isize[2], &normalize_factor, f_, 1);
        cublasCheckError (status);
    #else
        for (int i = 0; i < isize[0] * isize[1] * isize[2]; i++)
            f_[i] = f_[i] * normalize_factor;
    #endif

    /* Forward transform */
    executeFFTR2C (f_, f_hat_);
