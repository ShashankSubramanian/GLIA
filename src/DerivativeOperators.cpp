...
// /* #### ========    REACTION DIFFUSION FOR MOVING ATLAS (MA)       ======== #### */
// /* #### ------------------------------------------------------------------- #### */
// PetscErrorCode DerivativeOperatorsRDMovingAtlas::evaluateObjective (PetscReal *J, Vec x, Vec data) {
//     PetscFunctionBegin;
//     PetscErrorCode ierr = 0;
//     TU_assert (data != nullptr, "DerivativeOperatorsRDMovingAtlas::evaluateObjective: requires non-null input data.");
//     PetscScalar misfit_tu = 0, misfit_brain = 0;
//     PetscReal reg;
//     n_misc_->statistics_.nb_obj_evals++;
//
...
//     ierr = pde_operators_->solveState (0);                       CHKERRQ (ierr);
//     ierr = tumor_->obs_->apply (temp_, tumor_->c_t_);            CHKERRQ (ierr);
//     // compute m_tilde = mA(1)(1-c(1))
//     ierr = geometricCoupling(
//       tumor_->work_[7], tumor_->work_[8], tumor_->work_[9], tumor_->work_[10], tumor_->work_[11],
//       mA1_wm_, mA1_gm_, mA1_csf_, mA1_glm_,  mA1_bg_,
//       tumor_->c_t_, n_misc_);                                    CHKERRQ (ierr);
//     // evaluate tumor distance meassure || c(1) - d ||
//     ierr = VecAXPY (temp_, -1.0, data);                          CHKERRQ (ierr);
//     ierr = VecDot (temp_, temp_, &misfit_tu);                    CHKERRQ (ierr);
//     // evaluate brain tissue distance meassure || mT - mR ||, mT = mA(1)(1-c(1)), mR = patient
//     computeDifference(&misfit_brain,
//       xi_wm_, xi_gm_, xi_csf_, xi_glm_,  xi_bg_,
//       tumor_->work_[7], tumor_->work_[8], tumor_->work_[9], tumor_->work_[10], tumor_->work_[11],
//       mR_wm_, mR_gm_, mR_csf_, mR_glm_,  mR_bg_);                CHKERRQ (ierr);
//     // compute regularization
//     ierr = VecDot (tumor_->c_0_, tumor_->c_0_, &reg);            CHKERRQ (ierr);
//     reg *= 0.5 * n_misc_->beta_;
//     // compute objective function value
...
// }
//
// PetscErrorCode DerivativeOperatorsRDMovingAtlas::evaluateGradient (Vec dJ, Vec x, Vec data) {
//     PetscFunctionBegin;
//     PetscErrorCode ierr = 0;
//     PetscScalar misfit_brain;
//     n_misc_->statistics_.nb_grad_evals++;
