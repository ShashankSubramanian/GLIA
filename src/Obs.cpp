...
PetscErrorCode Obs::setDefaultFilter(Vec data, int time_point, ScalarType thr) {
  ...
  ScalarType th = 0;
  if (time_point == 0) {
    if (not two_snapshot_) {
      ierr = tuMSGwarn("Error: Cannot apply Obs(T=0), not a two snapshot scenario."); CHKERRQ(ierr);
    }
...
PetscErrorCode Obs::apply(Vec y, Vec x, int time_point, bool complement) {
  ...
  PetscErrorCode ierr;
  if (complement) {
    tuMSGstd(" applying complement obs op:  I-O ");
  }
  if (time_point == 0) {
    if (not two_snapshot_) {
      ierr = tuMSGwarn("Error: Cannot apply Obs(T=0), not a two snapshot scenario."); CHKERRQ(ierr);
    ...
    if (complement) {
      ierr = VecScale(filter_0_, -1); CHKERRQ(ierr);
      VecAXPY(filter_0_, 1, one); CHKERRQ(ierr);
    }
    ...
    if (complement) {
      ierr = VecScale(filter_1_, -1); CHKERRQ(ierr);
      VecAXPY(filter_1_, 1, one); CHKERRQ(ierr);
    }
