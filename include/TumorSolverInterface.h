...
struct DataDistributionParameters {
  ...
  DataDistributionParameters()
      : alloc_max(0), nlocal(0), nglobal(0), cdims{0, 0}, n{256, 256, 256}, istart{0, 0, 0}, isize{0, 0, 0}, ostart{0, 0, 0}, osize{0, 0, 0}, plan(nullptr), comm(MPI_COMM_WORLD), fft_mode(ACCFFT) {}
};
...
class TumorSolverInterface {
 public:
   ...
   */
  TumorSolverInterface(std::shared_ptr<Parameters> params = {}, std::shared_ptr<SpectralOperators> spec_ops = {}, std::shared_ptr<Phi> phi = {}, std::shared_ptr<MatProp> mat_prop = {});
   ...
   */
  PetscErrorCode initialize(DataDistributionParameters& ivars, std::shared_ptr<TumorParameters> tumor_params = {});
   ...
   *  @param MatProb[in] matprob - material properties, i.e., WM, GM, CSF
   */
  PetscErrorCode initialize(std::shared_ptr<Parameters> params, std::shared_ptr<SpectralOperators> spec_ops = {}, std::shared_ptr<Phi> phi = {}, std::shared_ptr<MatProp> mat_prop = {});
   ...
   */
  PetscErrorCode setParams(Vec p, std::shared_ptr<TumorParameters> tumor_params);

  PetscErrorCode setParams(std::shared_ptr<TumorParameters> tumor_params);
