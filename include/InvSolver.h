...
struct CtxCoSaMp {
  ...
  PetscErrorCode cleanup() {
    ...
    if (initialized) {
      ...
      if (x_full_prev != nullptr) {
        VecDestroy(&x_full_prev);
        x_full_prev = nullptr;
      }
      if (work != nullptr) {
        VecDestroy(&work);
...
struct CtxInv {
  ...
  CtxInv() : derivative_operators_(), params_(), tumor_(), data(nullptr), data_gradeval(nullptr), convergence_message(), cosamp_(nullptr) {
    ksp_gradnorm0 = 1.;
    gttol = 1e-3;
    gatol = 1e-6;
    grtol = 1e-12;
...
class InvSolver {
 public:
  InvSolver(std::shared_ptr<DerivativeOperators> derivative_operators = {}, std::shared_ptr<PdeOperators> pde_operators = {}, std::shared_ptr<Parameters> params = {},
  ...
  PetscErrorCode setParams(std::shared_ptr<DerivativeOperators> derivative_operators, std::shared_ptr<PdeOperators> pde_operators, std::shared_ptr<Parameters> params, std::shared_ptr<Tumor> tumor,
  ...
  void updateReferenceGradient(bool b) {
    if (itctx_ != nullptr) itctx_->update_reference_gradient = b;
  }
